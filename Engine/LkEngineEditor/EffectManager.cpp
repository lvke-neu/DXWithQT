...
EffectManager::EffectManager(void** parameter)
{
	...
	m_functions["setModelEffect"] = std::bind(&EffectManager::setModelEffect, this, std::placeholders::_1);
...
void* EffectManager::setModelEffect(void** parameter)
{
	...
	std::string modelPath = (*funcParameter)["modelPath"].GetString();
	...
	AssimpModelObject amo;
	amo = AssimpModelObject(modelPath, gameApp->getScene()->getD3dDevice(), gameApp->getScene()->getD3dImmediateContext());
	amo.setShader(SceneShader::shaderPath[1]);
	amo.setMaterial(material);
	amo.setTransform(Transform(
		scale,
		rotation,
		position
	));
