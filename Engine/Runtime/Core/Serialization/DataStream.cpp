...
namespace Twinkle
{
    ...
    DataStream::DataStream(const std::string& str) : m_pos(0)
    {
        m_byteorder = byteorder();
        m_buf.clear();
        reserve((int)str.size());
        write(str.data(), (int)str.size());
    }

    void DataStream::reserve(int len)
    {
        int size = (int)m_buf.size();
        int cap = (int)m_buf.capacity();
        if (size + len > cap)
        {
            while (size + len > cap)
            {
    ...
    void DataStream::write(const SerializationObject& value)
    {
    ...
    bool DataStream::read(SerializationObject& value)
    {
    ...
    DataStream& DataStream::operator << (const SerializationObject& value)
    {
        write(value);
        return *this;
    }
    ...
    DataStream& DataStream::operator >> (SerializationObject& value)
    {
        read(value);
        return *this;
    }
