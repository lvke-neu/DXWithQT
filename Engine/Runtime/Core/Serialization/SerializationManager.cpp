...
namespace Twinkle
{
	...
	void to_json_recursively(const instance& obj2, PrettyWriter<StringBuffer>& writer)
	{
		...
		//writer the classType
		writer.String("classType");
		...
		for (auto prop : prop_list)
		{
			...
			if (!write_variant(prop_value, writer))
			{
				...
				LOG_ERROR("cannot serialize property: " + name);
			}
		}
	...
	void SerializationManager::Serialize(rttr::instance obj_In, std::string & jsonStr_Out)
	{
		jsonStr_Out = "";
		if (!obj_In.is_valid())
			return ;
		...
		to_json_recursively(obj_In, writer);

		jsonStr_Out = sb.GetString();
	}
	...
	void SerializationManager::UnSerialize(std::string & jsonStr_In, rttr::instance obj_Out)
	{
		Document document; 
		if (document.Parse(jsonStr_In.c_str()).HasParseError())
			return;

		fromjson_recursively(obj_Out, document);
	}
}